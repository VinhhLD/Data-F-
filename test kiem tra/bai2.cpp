...
void showBRTNode(BRTNode* p)
{
	...
	printf("%s  %30s  %10s  %10d\n", p->Info.maSach ,p->Info.tenSach,p->Info.nhaXB,p->Info.namXB);
...
BRTNode* BRTInsert(BRTNode* root, BRTNode* pNew)
{
	...
	if (_strcmpi(pNew->Info.maSach, root->Info.maSach) < 0)
	{
		root->Left = BRTInsert(root->Left, pNew);
		root->Left->Parent = root;
	}
	else if (_strcmpi(pNew->Info.maSach, root->Info.maSach) > 0)
	{
...
BRTNode* search(BRTNode* root, AV x) {
	BRTNode* temp = root;
	while (temp != NULL) {
		if (_strcmpi(x.maSach, temp->Info.maSach) == 0) {
			return temp;
		}
		else if (_strcmpi(x.maSach, temp->Info.maSach) < 0)
		{
			temp = temp->Left;
		}
		else {
...
void deleteByInfo(BRTNode*& root, AV x) {
	...
	if (delete_pt == NULL) {
		printf("\nKhong tim thay nut nao de xoa voi gia tri: %s", x.maSach);
		return;
	}
...
{//Ham tao cay Black-Red Tree tu mang a
	...
	AV a;
	int n;
	FILE* f = fopen("doden.txt", "rt");
	fscanf(f, "%d", &n);
	fgetc(f);
	for (int i = 0; i < n; i++)
	{
		fscanf(f, "%[^#]%*c%[^#]%*c%[^#]%*c%d\n", &a.maSach, &a.tenSach, &a.nhaXB, &a.namXB);
		insert(brt.Root, a);
		set_Color(14, 2);
