...
void showBRTNode(BRTNode* p)
{
	...
	printf("%-10s%-40s%-25s%-10d\n", p->Info.masach, p->Info.tensach, p->Info.nhaxb, p->Info.nam);
...
BRTNode* BRTInsert(BRTNode* root, BRTNode* pNew)
{
	...
	if (stricmp(pNew->Info.masach, root->Info.masach) < 0)
	{
		root->Left = BRTInsert(root->Left, pNew);
		root->Left->Parent = root;
	}
	else if (stricmp(pNew->Info.masach, root->Info.masach) > 0)
	{
...
void insert(BRTNode*& root, Sach Info)
{
...
//	if (root == NULL) return;
//
//	BRTNode* delete_pt = searchNode(root, x);
//
//	if (delete_pt == NULL) {
//		printf("\nKhong tim thay nut nao de xoa voi gia tri: %d", x);
//		return;
//	}
//	printf("Xoa thanh cong nut co gia tri %d\n", x);
...
//	if (!root)
//		return 0;
//	int nl = CountBRTree(root->Left);
//	int nr = CountBRTree(root->Right);
//	return 1 + nl + nr;
//}
...
//		return 1;
//	k--;
//	int nl = CountBRTreeLevelK(root->Left, k);
//	int nr = CountBRTreeLevelK(root->Right, k);
//	return nl + nr;
//}
...
//		return 0;
//	}
//	int nl = CountBRTreeRed(root->Left);
//	int nr = CountBRTreeRed(root->Right);
//	if (isNodeRed(root) == true) return 1;
//	return nl + nr;
//}
...
//	if (!root)
//		return 0;
//	int nl = SumBRTree(root->Left);
//	int nr = SumBRTree(root->Right);
...
void Menu()
{
	...
	printf("\n5. Xoa nut x.																");
	printf("\n6. Tim nut x.																");
	printf("\n7. Dem so nut do den tren cay.											");
	printf("\n8. Dem so nut do den tren cay o muc k.									");
	printf("\n9. Tong gia tri nut trong cay.											");
	printf("\n10.Them 1 nut x co gia tri bat ky vao cay									");
	printf("\n11. Xoa 1 nut co gia tri x bat ky cua cay									");
	printf("\n12. Tim kiem mot gia tri x co tren cay hay khong							");
	printf("\n13. Dem nut do trong cay													");
	printf("\n14. Dem nut den trong cay													");
	printf("\n15. Doc file																");
	printf("\n==========================================================================");
}
void createBRTree(BRTree& brt, FILE*& f)
{//Ham tao cay Black-Red Tree tu mang a
	initBRTree(brt);
	Sach a;
	int n;
	...
	for (int i = 0; i < n; i++)
	{
		fscanf(f, "%[^#]%*c%[^#]%*c%[^#]%*c%d\n", a.masach, a.tensach, a.nhaxb, &a.nam);
		insert(brt.Root, a);
		set_Color(14, 2);
	}
	/*while (!feof(f))
	{
		fscanf(f, "%s%s", a.anh, a.viet);
		insert(brt.Root, a);
		set_Color(14, 2);
